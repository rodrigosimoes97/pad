...
export function loadSettings(): PadSettings {
  try {
    ...
    return {
      ...
      reverseMix: isNumber(parsed.reverseMix) ? Math.min(0.7, Math.max(0, parsed.reverseMix)) : DEFAULT_SETTINGS.reverseMix,
      ...
      reverseDebugSolo: typeof parsed.reverseDebugSolo === 'boolean' ? parsed.reverseDebugSolo : DEFAULT_SETTINGS.reverseDebugSolo,
      layers: {
